...
import { adminListProposals, adminPatchProposal, type Proposal } from "@/lib/api";
...
export default function AdminProposalsPage() {
  ...
  const patch = async (id: string, status: Proposal["status"]) => {
    try {
      setSavingId(id);
      await adminPatchProposal(id, { status });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold">Proposals Moderation</h1>
        ...
        <table className="min-w-full text-sm text-slate-200">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={3} className="px-4 py-6 text-center text-slate-400">No proposals</td></tr>
            ) : (
              items.map((p) => (
                <tr key={p.id} className="border-t border-slate-800">
                  <td className="px-4 py-3 align-top">
                    <div className="font-medium text-slate-100">{p.id}</div>
                    <div className="text-xs text-slate-400">Job: {p.jobId} • Bid: {p.bidAmount} • Days: {p.timelineDays}</div>
                  </td>
                  <td className="px-4 py-3 align-top">{p.status}</td>
                  <td className="px-4 py-3 align-top">
                    <div className="flex flex-wrap gap-2">
                      {(["SUBMITTED","SHORTLISTED","ACCEPTED","REJECTED","WITHDRAWN"] as Proposal["status"][]).map((s) => (
                        <button
                          key={s}
                          disabled={savingId === p.id || p.status === s}
                          onClick={() => patch(p.id, s)}
                          className={`rounded-md border px-2 py-1 text-xs ${p.status === s ? "cursor-not-allowed border-slate-700 bg-slate-800 text-slate-400" : "border-slate-700 bg-slate-800 text-slate-200 hover:bg-slate-700"}`}
                        >
                          {s}
                        </button>
                      ))}
