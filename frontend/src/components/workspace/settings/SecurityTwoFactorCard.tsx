...
export function SecurityTwoFactorCard({ onProfileChange, profile }: Props) {
  ...
  const emailReadyForEnable = !usesEmail || Boolean(profile.emailVerified) || /^\d{6}$/.test(emailOtp.trim());
  const phoneReadyForEnable = !usesPhone || Boolean(profile.phoneVerified) || /^\d{6}$/.test(phoneOtp.trim());
  ...
  const handleGenerateAuthenticatorSetup = async () => {
    setIsBusy(true);
    try {
      const response = await beginAuthenticatorSetup();
      setAuthenticatorSetup(response.setup);
      onProfileChange(response.profile);
      toast.success("Authenticator QR and secret generated.");
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Failed to generate authenticator setup.");
    } finally {
      setIsBusy(false);
    }
  };
    ...
    <section className="rounded-[28px] border border-gray-200/80 bg-white p-6 shadow-[0_18px_44px_rgba(15,23,42,0.04)]">
      ...
      <div className="mt-5 grid gap-5">
          ...
          <div className="grid gap-4 rounded-[24px] border border-gray-200 bg-gray-50 p-5 lg:grid-cols-[180px_minmax(0,1fr)]">
            ...
            <div className="grid gap-3">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <button
                  type="button"
                  onClick={() => void handleGenerateAuthenticatorSetup()}
                  disabled={isBusy}
                  className="rounded-full border border-gray-200 bg-white px-4 py-2 text-sm font-semibold text-gray-700 disabled:opacity-60"
                >
                  {authenticatorSetup ? "Regenerate QR / key" : "Generate QR / key"}
                </button>
                <span className="text-xs text-gray-500">If QR does not appear, regenerate and use the new secret.</span>
              </div>
              {authenticatorSetup ? (
                <>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          {(!emailReadyForEnable || !phoneReadyForEnable) && (usesEmail || usesPhone) ? (
            <p className="text-xs text-amber-700">Enter valid 6-digit email/phone OTP codes for selected unverified channels.</p>
          ) : null}
        </div>
