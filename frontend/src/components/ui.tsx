...
import { Avatar as MuiAvatar, Box, Chip, CircularProgress, Dialog, DialogActions, DialogContent, DialogTitle, FormControl, LinearProgress, MenuItem, Paper, Select as MuiSelect, Skeleton as MuiSkeleton, Tab, Tabs as MuiTabs, Table as MuiTable, TableBody, TableCell, TableContainer, TableHead, TableRow, type TextFieldProps, Typography,
} from "@mui/material";
...
}: React.HTMLAttributes<HTMLDivElement> & { variant?: "default" | "bordered" | "elevated"; hover?: boolean;
}) { return ( <SoftCard className={className} sx={{ border: variant === "bordered" ? "2px solid" : "1px solid", borderColor: "divider", boxShadow: variant === "elevated" ? 5 : 2, transition: "all 180ms ease", ...(hover ? { cursor: "pointer", "&:hover": { transform: "translateY(-2px)", boxShadow: 8, }, } : {}), }} {...props} > {children} </SoftCard> );
...
} export function Button({ children, variant = "primary", size = "md", className, isLoading, leftIcon, rightIcon, ...props
}: Omit<React.ButtonHTMLAttributes<HTMLButtonElement>, "color"> & { variant?: "primary" | "secondary" | "outline" | "danger"; size?: "sm" | "md" | "lg"; isLoading?: boolean; leftIcon?: ReactNode; rightIcon?: ReactNode;
}) { const muiVariant: "outlined" | "contained" = variant === "outline" ? "outlined" : "contained"; const muiColor: "error" | "secondary" | "primary" = variant === "danger" ? "error" : variant === "secondary" ? "secondary" : "primary"; return ( <SoftButton variant={muiVariant} color={muiColor as "error" | "secondary" | "primary"} size={size === "lg" ? "large" : size === "sm" ? "small" : "medium"} className={className} disabled={Boolean(props.disabled || isLoading)} startIcon={isLoading ? <CircularProgress size={16} color="inherit" /> : leftIcon} endIcon={!isLoading ? rightIcon : undefined} {...props} > {isLoading ? "Loading..." : children} </SoftButton> );
