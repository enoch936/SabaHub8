...
start_process_tmux "frontend" "frontend" "$ROOT_DIR/frontend" "exec npm run dev -- --port 3000" "$FE_PID_FILE" "$LOG_DIR/frontend.log"
wait_http "frontend" "http://127.0.0.1:3000" 60 1 || true
...
echo
echo "All main project processes are started in tmux session '$SESSION_NAME'."
echo "Attach for live logs: tmux attach -t $SESSION_NAME"
echo "Window names: python-ai, backend-spring, frontend"
echo "Logs: $LOG_DIR"
