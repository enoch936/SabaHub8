...
export function StreamVideoStage({
  ...
  const [source, setSource] = useState(primarySrc || fallbackSrc || "");
  ...
  useEffect(() => {
    ...
    setSource(primarySrc || fallbackSrc || "");
    setUsingFallback(!primarySrc && Boolean(fallbackSrc));
    setIsPlaying(false);
    setStatusNote("");
  }, [fallbackSrc, mediaStream, primarySrc]);
  ...
  const handleError = () => {
    if (!usingFallback && fallbackSrc && source !== fallbackSrc) {
      setSource(fallbackSrc);
      setUsingFallback(true);
      setStatusNote("Live feed is not ready yet. Showing a real demo video for now.");
      return;
    }

    setStatusNote("Video is not available right now.");
  };
        ...
        <div className={`relative ${aspectClassName}`}>
          {hasRenderableSource ? (
            <video
              ...
              muted={muted}
              controls
              loop={!hasDeviceStream && usingFallback}
              playsInline
      ...
      <div className="flex flex-wrap items-center gap-2 text-xs text-gray-500">
        <span
          className={`rounded-full px-3 py-1 font-semibold ${
            usingFallback ? "bg-amber-50 text-amber-700" : "bg-emerald-50 text-emerald-700"
          }`}
        >
          {hasDeviceStream ? "Live device source" : usingFallback ? "Playable demo clip" : "Stream playback"}
        </span>
