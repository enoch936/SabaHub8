...
export const SmartJobCard = memo(function SmartJobCard({
  ...
  return (
    <div
      className={`group rounded-[16px] bg-white overflow-hidden shadow-sm hover:shadow-md transition ${
        viewMode === "list" ? "" : ""
      }`}
    >
      <div className={`${viewMode === "list" ? "flex flex-col xl:flex-row" : "flex flex-col"}`}>
            ...
            <div className="relative overflow-hidden bg-[var(--accent)] transition duration-200 group-hover:shadow-md">
              {activeMedia.kind === "image" ? (
                <img
                  ...
                  className={`w-full object-cover transition duration-200 group-hover:scale-105 ${viewMode === "list" ? "h-48" : "h-32"}`}
                  loading="lazy"
                />
              ) : (
                <video
                  ...
                  controls
                  preload="metadata"
                  className={`w-full object-cover bg-black ${viewMode === "list" ? "h-48" : "h-32"}`}
                />
              )}
        ...
        <div className="flex-1 p-3">
          <div className="flex items-start justify-between gap-1.5">
            <div className="min-w-0 flex-1">
              <h3 className="text-xs font-semibold leading-tight text-gray-900 line-clamp-2">
                {job.title}
              </h3>
              <div className="mt-1.5 flex items-center gap-1">
                <span className="inline-flex items-center gap-0.5 rounded-full bg-white px-1.5 py-0.5 text-[8px] font-medium text-slate-700 border border-gray-200">
                  {job.budget.type === "FIXED"
                    ? `$${(job.budget.min / 1000).toFixed(0)}k`
                    : `$${job.budget.min}/hr`}
                </span>
                {job.employerVerified && <CheckCircle className="h-3 w-3 text-blue-500" />}
              </div>
