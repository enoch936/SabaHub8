...
export const SmartJobCard = memo(function SmartJobCard({
  ...
  if (viewMode === "list") {
    return (
      <div className="group flex overflow-hidden rounded-lg bg-white shadow-sm transition-all duration-200 hover:shadow-2xl hover:bg-gray-200">
        <div className="flex w-full flex-col sm:flex-row">
          <div className="h-40 w-full flex-shrink-0 sm:h-32 sm:w-32">
            {activeMedia ? (
  ...
  return (
    <div
      className={`group rounded-[12px] bg-white overflow-hidden shadow-sm hover:shadow-2xl hover:bg-gray-200 transition-all duration-200`}
    >
      <div className={`flex flex-col`}>
        <div>
          {activeMedia ? (
            <div className="relative overflow-hidden bg-[var(--accent)] transition duration-200 group-hover:shadow-md">
              {/* YouTube-style dark overlay on hover */}
              <div className="absolute inset-0 z-10 bg-black/0 group-hover:bg-black/40 transition-colors duration-200 pointer-events-none" />
              {activeMedia.kind === "image" ? (
                <img
                  src={activeMedia.url}
                  alt={`${job.title} media ${mediaIndex + 1}`}
                  className={`w-full h-32 object-cover transition duration-200 group-hover:scale-110`}
                  loading="lazy"
                />
              ) : (
                <video
