  ...
  return (
    ...
    >
      <div className={`${viewMode === "list" ? "flex flex-row gap-4 p-4" : "flex flex-col"}`}>
            ...
            {viewMode === "list" && (
              <div className="mt-3 flex flex-wrap items-center gap-2">
                <button
                  type="button"
                  onClick={() => onApply(job)}
                  className="inline-flex items-center gap-1 rounded-lg bg-gray-950 px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-gray-800"
                >
        ...
        {viewMode === "list" && (
          <div className="hidden lg:block lg:w-1/3 lg:flex-shrink-0 bg-gradient-to-l from-gray-50 via-gray-50 to-transparent" />
        )}
      </div>
    </div>
  );
