...
export function JobCardGrid({ jobs, viewMode, onApply, onSave, onDelete, onMessage }: JobCardGridProps) {
  ...
  return (
    <div className={viewMode === "grid" ? "grid grid-cols-1 gap-2 xs:grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5" : "space-y-2"}>
      {jobs.map((job) => (
        <div key={job.id}>
          <SmartJobCard
            job={job}
